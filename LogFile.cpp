...
std::string CFileUtils::RetrievePath()
{
    QString  strAppDirPath = QCoreApplication::applicationDirPath(); // QCoreApplication::applicationFilePath();
    std::string sResult = strAppDirPath.toStdString();
    return sResult;
}

std::string CFileUtils::RetrieveParentPath()
{
    std::string sPath = CFileUtils::RetrievePath() + "/";
    if (sPath == "-1")
        return sPath;
...
std::string CFileUtils::RetrieveFileName(const std::string& sFileName)
{
    int iSize = sFileName.size();

    char* szString = new char[iSize + 1];
    memset(szString, 0x20, iSize); // Space symbols
    szString[iSize] = '\0';

    int iIndex = iSize;
    do
    {
        char ch = sFileName[iIndex];
        if (ch != 0x2F) // symbol "/"
            szString[iIndex--] = ch;
        else
            break;
    ...
    std::string sOutResult;
    for (int i = 0; i <= iSize; i++)
    {
        char chOutChar = szString[i];
        if ((chOutChar > 0) && (chOutChar != 0x20))
            sOutResult.push_back(chOutChar);
    }
    delete[] szString;
...
void CLogFile::SaveLogFile(LOG_MODES Category, const char* pszFormat, ...)
{
    ...
    char szLog[8192];  // Corrected 12.02.2020
	va_list argList;
